...
threeCol2listMat<- function(a = c("a","b","c","c"), b = c("a","b","b","b"), v = c(1,2,3,4)){
	print ("threeCol2listMat is called")
	if(length(a)!=length(b) | length(a)!=length(v)){
		return(-1)
	}
	a1 = as.character(a)
...
#' @param stage2 (b=boolean): run stage 2. Default is FALSE.
#' @keywords random walk with restart
#' @import Matrix
...
#	library(ROCR)
	...
	features = unique(edges[which(edges$type %in% prop_etypes),c("target","type")])
	featnodes = as.character(features[,"target"])
	...
	for (restart in restarts){
		...
		if(!file.exists(basefile)){
			...
			biter = rwr_res$iter
#			evaltabstart = cbind(evaltabstart,round(as.numeric(rwr_res$vec),digits = 10))
			evaltabstart = cbind(evaltabstart,formatC(as.numeric(rwr_res$vec), format = "e", digits = 4))
			colnames(evaltabstart)[4] = "baseline"

			if(!file.exists(basefile)){
				write.table(evaltabstart[featnodes,], basefile, quote=F, sep="\t", row.names=T, col.names=NA)
			}
		...
		for (qfile in possets){
			...
			for(iter in 1:nfolds){
				...
				testuni = as.character(c(test_nidxs,sample(setdiff(setdiff(uniIDs,train_nidxs),test_nidxs),ntest)))
				...
				# store results
				qiter = rwr_res$iter
#				evaltab = cbind(evaltabstart,0,0,round(as.numeric(rwr_res$vec[nodenames]),digits = 10))
				evaltab = cbind(evaltabstart,0,0,formatC(as.numeric(rwr_res$vec[nodenames]), format = "e", digits = 4))
				...
				if(iter == 1 && writepreds){
					write.table(evaltab[featnodes,7], outfile, quote=F, sep="\t", row.names=F, col.names=T)
				}
